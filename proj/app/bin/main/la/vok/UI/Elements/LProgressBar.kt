...
class LProgressBar(
    ...
    override fun updateSprites() {
        ...
        if (fillImageKey.isNotEmpty()) {
            fillSprite = Storage.gameController.loaders.sprites.getSprite(fillImageKey)
        }
        if (backgroundImageKey.isNotEmpty()) {
            backgroundSprite = Storage.gameController.loaders.sprites.getSprite(backgroundImageKey)
        }
