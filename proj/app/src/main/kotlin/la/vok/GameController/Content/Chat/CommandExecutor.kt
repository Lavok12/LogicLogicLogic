...
class CommandExecutor(
    ...
    fun handleCommand(autorName: String, rawText: String, sendMessage: (String, ChatMessage) -> Unit) {
        ...
        if (!gameController.commandsLoader.hasCommand(commandName)) {
            sendMessage(autorID, errorMessage("Неизвестная команда: $commandName"))
            return
        }

        val argTypes = argsClassifier.classifyArgs(args)
        val commandVariation = gameController.commandsLoader.getCommandVariation(commandName, argTypes)
        ...
        confObject.configuratorId = gameController.commandsLoader.getCommandVariationIndex(commandName, confObject.classifyArgs)
